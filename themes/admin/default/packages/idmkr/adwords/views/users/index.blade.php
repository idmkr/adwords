...
@section('title')
@parent
{{ trans('idmkr/adwords::users/common.title') }}
@stop
...
{{ Asset::queue('index', 'idmkr/adwords::users/js/index.js', 'platform') }}
...
<section class="panel panel-default panel-grid">
		...
		<nav class="navbar navbar-default navbar-actions">
			...
			<div class="container-fluid">
				...
				<div class="navbar-header">
					...
					<span class="navbar-brand">{{{ trans('idmkr/adwords::users/common.title') }}}</span>
				...
				<div class="collapse navbar-collapse" id="actions">
					...
					<ul class="nav navbar-nav navbar-left">
						...
						<li class="primary">
							<a href="{{ route('admin.idmkr.adwords.users.create') }}" data-toggle="tooltip" data-original-title="{{{ trans('action.create') }}}">
					...
					{{-- Grid: Filters --}}
					<form class="navbar-form navbar-right" method="post" accept-charset="utf-8" data-search data-grid="user" role="form">
						...
						<div class="input-group">
							...
							<span class="input-group-btn">
								...
								<ul class="dropdown-menu" role="menu">
									...
									<li>
										<a data-grid="user" data-filter="enabled:1" data-label="enabled::{{{ trans('common.all_enabled') }}}" data-reset>
											<i class="fa fa-eye"></i> {{{ trans('common.show_enabled') }}}
										</a>
									</li>

									<li>
										<a data-toggle="tooltip" data-placement="top" data-original-title="" data-grid="user" data-filter="enabled:0" data-label="enabled::{{{ trans('common.all_disabled') }}}" data-reset>
	...
	<div class="table-responsive">
		...
		<table id="data-grid" class="table table-hover" data-source="{{ route('admin.idmkr.adwords.users.grid') }}" data-grid="user">
			<thead>
				<tr>
					<th><input data-grid-checkbox="all" type="checkbox"></th>
					<th class="sortable" data-sort="id">{{{ trans('idmkr/adwords::users/model.general.id') }}}</th>
					<th class="sortable" data-sort="client_manager_id">{{{ trans('idmkr/adwords::users/model.general.client_manager_id') }}}</th>
					<th class="sortable" data-sort="user_id">{{{ trans('idmkr/adwords::users/model.general.user_id') }}}</th>
					<th class="sortable" data-sort="client_customer_id">{{{ trans('idmkr/adwords::users/model.general.client_customer_id') }}}</th>
					<th class="sortable" data-sort="created_at">{{{ trans('idmkr/adwords::users/model.general.created_at') }}}</th>
				</tr>
			</thead>
			<tbody></tbody>
		</table>
	...
	<footer class="panel-footer clearfix">
		...
		{{-- Grid: Pagination --}}
		<div id="data-grid_pagination" data-grid="user"></div>

	</footer>

	{{-- Grid: templates --}}
	@include('idmkr/adwords::users/grid/index/results')
	@include('idmkr/adwords::users/grid/index/pagination')
	@include('idmkr/adwords::users/grid/index/filters')
	@include('idmkr/adwords::users/grid/index/no_results')

</section>

@if (config('platform.app.help'))
	@include('idmkr/adwords::users/help')
@endif
